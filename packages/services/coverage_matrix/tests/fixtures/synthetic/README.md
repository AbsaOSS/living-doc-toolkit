...
- `collision_doc_source.json` / `collision_ui_tests.json` — two User Stories that share
  ...
  and a scenario for an unresolved User Story (`US-5`). The matcher disambiguates the
  collision on the scenario's `source` org/repo, so the `absa-group` scenario covers the
  `absa-group` US-1 and does not leak onto the `other-org` one. A scenario whose `source`
  matches neither colliding US would be left unresolved (unlinked) rather than guessed at.
