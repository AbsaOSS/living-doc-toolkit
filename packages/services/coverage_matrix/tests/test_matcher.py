...
def test_cross_source_short_id_collision_disambiguates_on_source():
    doc = _doc(
        [
    ...
    tests = [
        _scenario("s1", "US-1", ["US-1-01"], source={"org": "org-a", "repo": "repo-a", "file": "f.feature"}),
        _scenario("s2", "US-1", ["US-1-02"], source={"org": "org-b", "repo": "repo-b", "file": "f.feature"}),
    ]
    ...
    assert by_full["org-b/repo-b/US-1"].acceptance_criteria[0].coverage.status == "covered"
    assert matrix.stale_ac_refs == []
...
def test_cross_source_collision_with_unmatched_source_is_unresolved():
    ...
    tests = [_scenario("s1", "US-1", ["US-1-01"], source={"org": "elsewhere", "repo": "x", "file": "f.feature"})]
