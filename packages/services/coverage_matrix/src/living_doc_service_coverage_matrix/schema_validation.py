...
Validates a full doc-source envelope (carrying ``metadata`` and ``warnings``) against
the bundled ``doc-source-v1.0.0`` JSON Schema before the matcher consumes it; a bare
JSON array or a legacy ``items`` envelope is not schema-validated. That schema is a
vendored, pinned copy owned and generated by ``living-doc-collector-gh`` — see
``schema/README.md``.
"""
