...
You are driving a single roadmap task to completion in `living-doc-toolkit`.
...
- Prefer a change that touches one package. Split a cross-package change unless the parts
...
code / config that now exists — **not** by checking that a same-named test is green.
...
- "deprecated ACs excluded from `coverage_pct`" → confirm the filter precedes the division
  in `summary.py`.
- "`requires-python` is `>=3.10`" → open each `pyproject.toml` and read the literal value.
